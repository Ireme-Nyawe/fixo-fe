...
const TechnicianCallView: React.FC<TechnicianCallViewProps> = ({
  ...
  useEffect(() => {
    const handleOffline = () => {
      ...
      setNetwork(false);
      socket?.emit("network-lost", { to: user.userId });
    };
    ...
    if (isScreenSharing) {
      ...
      try {
        ...
        if (!navigator.mediaDevices.getDisplayMedia) {
          alert(
            "Screen sharing is not supported on this device, Try With Computer!"
          );
        }
        if (localVideoRef.current) {
          localVideoRef.current.srcObject = stream;
        ...
        <div className="max-w-7xl mx-auto bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 md:p-8">
          ...
          <div
            className={`grid grid-cols-1 ${
              !techFullScreen && !userFullScreen ? "md:grid-cols-2" : ""
            } gap-4 sm:gap-6 md:gap-8 mb-6 sm:mb-8`}
          >
            <div
              ref={techVideoContainerRef}
              className={`relative bg-gray-900 rounded-xl sm:rounded-2xl overflow-hidden aspect-video border-2 sm:border-4 border-gray-200 ${
            ...
            >
              ...
              <div
                ...
                } bg-gray-800 bg-opacity-80 text-gray-100 px-2 py-1 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-medium`}
              >
